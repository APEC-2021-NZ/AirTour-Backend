...
const generateBookingMessage = (startDate, endDate, numTourists, content) => {
    return `[Booking Request] From ${dateFormat(startDate)} to ${dateFormat(
        endDate,
    )} for ${numTourists} ${numTourists > 1 ? 'people' : 'person'}.`
}
...
const ConversationResolver = {
    Query: {
        conversation: async (parent, { conversationID }, context, info) => {
            ...
            if (
                ![conversation.userID, conversation.guideID].includes(
                    context.user.uid,
                )
            )
    ...
    Mutation: {
        createConversation: async (parent, { input }, context, info) => {
            ...
            let conversationId = await Fireo.runTransaction(
                async (transaction) => {
                    ...
                    if (existingConversation)
                        throw new ConversationExistsError()
                    ...
                    let systemMessage = Message.init({
                        parent: conversation.key,
                    })
                    systemMessage.from = user.key
                    systemMessage.content = generateBookingMessage(
                        startDate,
                        endDate,
                        numTourists,
                        content,
